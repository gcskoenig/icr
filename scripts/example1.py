...
# INTERLUDE: TESTS OF THE RECOURSE FUNCTIONS; TAKE OUT EVENTUALLY
...
for r_type in r_types:
    for t_type in t_types:
        ...
        logging.info('Accuracy on recourse seeking pre-recourse: {}'.format(accuracy_pre))
        logging.info('Accuracy on recourse seeking post-recourse: {}'.format(accuracy_post))


        logging.info('The recourse strategies for all possible input combinations are computed and logging.infoed:')
        strategies = df_rec1[
            ['presymptom-free', 'prevaccinated', 'int symptom-free', 'int vaccinated']].drop_duplicates()

        logging.info('Recourse strategy:')
        ...
        logging.info('Feature names: {}'.format(X.columns))
        for ii in range(len(data)):
            logging.info('prediction for {}: {}'.format(data[ii], predict_all_comb[ii]))
